...
export const registerSpecInitPrompt = (server: McpServer) => {
  server.registerPrompt(
    "spec.init",
    {
      ...
      description:
        "仕様駆動開発用のディレクトリを初期化し、後続プロンプトの実行手順を案内する。",
      argsSchema: {
        specDescription: z
          .string()
        ...
        presetSlug: z
          ...
          .optional()
          .describe(
            "任意。フォルダ名を明示指定する場合に利用。重複時はユニーク化されます。",
          ),
        locale: z
    ...
    async ({ specDescription, presetSlug, locale }) => {
      ...
      const suggested = normalizedPreset && normalizedPreset.length > 0 ? normalizedPreset : suggestions[0];
